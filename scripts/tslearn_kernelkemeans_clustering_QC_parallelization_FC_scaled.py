...
def worker_init(
    ...
    global GLOBAL_N_INIT, GLOBAL_KERNEL_METRIC, GLOBAL_GAK_SIGMA #, GLOBAL_MAX_ITER
...
def main() -> None:
    ...
    if args.gak_sigma is not None:
        ...
        from utils import reshape_df
        column_names = df_sample.columns.tolist()
        column_selection = [
            c for c in column_names
            if c.startswith(args.data_type)
               and any(cond in c for cond in args.conditions)
               and "full" not in c
        ]
